...
Key Insight:
- Resolution is **top-down hierarchical**, but **rarely reaches root** due to caching.
- This caching at multiple layers is why DNS is **highly scalable and resilient**.
```
...
- **Monitors** response times, CPU load, connection count, etc. (for smart LBs).
 - Active vs Passive Health Checks:
   ...
   - LB periodically calls a health endpoint (e.g., /health, /status)
   - Works even without user traffic
  ...
  In production: BOTH are used together. “Passive detects real issues, active detects silent failures.”
...
#### L7 Overhead (Why it's heavier)

- L7 LB must:
  1. Terminate TLS (decrypt traffic)
  2. Inspect HTTP layer (headers, cookies, URL)
  3. Apply routing logic
  4. Re-encrypt traffic

→ This adds CPU overhead and latency

Whereas L4:
- Just forwards TCP packets
- No inspection → faster, cheaper
---
...
- Cryptographic hashes (SHA-256, MD5):
  ❌ Too slow for distributed systems
...
```
<img width="1536" height="1024" alt="image" src="https://github.com/user-attachments/assets/9df318a8-58a3-425b-8dbe-8697849a1951" />
...
<img width="1536" height="1024" alt="image" src="https://github.com/user-attachments/assets/15484d86-35df-4972-833f-e33cb654e1dd" />
...
```
### Why VNodes Matter (Failure Distribution)

Without VNodes:
DB3 fails → 100% load → DB4 ❌ (hotspot)
...
→ Load spreads across multiple servers ✅

Key Insight:
- VNodes convert **localized failure → distributed impact**
...
In decentralized systems (e.g., Cassandra):

- No central master tracks the ring
...
### Real-World Optimization: Bounded Load (Advanced)

Problem:
Even with consistent hashing + VNodes, imbalance can occur.

Solution:
- Use "Power of Two Choices":
  - Pick 2 candidate servers
