...
* **Definition**: A reserved disk area to extend RAM virtually.
* **Types**:
...
* **Example Sizes**:
  ...
  * 8 GB RAM → 10 GB Swap (older recommendation).
  * Modern: Swap often = RAM size or less (depends on workload).

### Swap Operations

* **Swap In** → Data moved from disk (swap) → RAM.
* **Swap Out** → Data moved from RAM → disk (swap).
...
1. **free -h** → Check free memory & swap in human-readable form.
2. **top** / **htop** → Monitor live CPU & memory usage.
3. **/proc/meminfo** → Detailed memory statistics.
4. **vmstat** → Shows `procs`, `memory`, `swap`, etc.
...
* `free -m | awk 'NR==2 {print $4}'` → Extracts **free column** from the 2nd line (RAM).
