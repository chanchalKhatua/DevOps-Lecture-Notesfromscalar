...
Perfect — you're setting up **Slack integration** for Alertmanager notifications!  
---

### 🔔 Alertmanager Slack Notification Example
...
receivers:
  - name: slack
    slack_configs:
      - channel: "#temp-devops-assignment-1"  # Slack channel where alerts will be posted
        api_url: "https://hooks.slack.com/services/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"  # Incoming Webhook URL
        text: |
          *Alert:* {{ .CommonAnnotations.summary }}
          {{ .CommonAnnotations.description }}
        send_resolved: true  # Send a message when the alert is resolved too
```
...
- **route.receiver**: 
  - The default alert receiver is `slack`.
  
- **receivers.slack_configs**:
  - Posts alerts to **`#temp-devops-assignment-1`** Slack channel.
  - Uses the provided **Slack Incoming Webhook URL**.
  - Formats the alert message using **annotations** (`summary` and `description` from your alert rules).
  - **send_resolved: true**: Sends another Slack message once the alert gets resolved.

---

### ⚡ Quick Important Notes:
- Make sure your **Slack webhook URL** is active and linked to the correct workspace and channel.
- Ensure your **Prometheus alert rules** (like the `HighCPUUsage` you created) have **annotations** with `summary` and `description` — otherwise the Slack message will be empty.
...
### Purpose
  ...
  - Capture Ephemeral Data: Short-lived processes push their metrics because by the time Prometheus scrapes, the
     process might have already terminated.
  - Flexibility: It decouples metric generation from the scrape cycle, making your monitoring more adaptable to dynamic
     and unpredictable workloads.
  - Temporary Storage: Metrics are held temporarily until Prometheus scrapes them, ensuring that even fleeting data is
     not lost.
### How It Works
 - Metric Generation:
   - A job or application collects metrics and pushes them using an HTTP-based protocol.
 - Temporary Storage:
   - The Pushgateway receives these metrics and stores them temporarily until the next scrape.
 - Scraping:
   - Prometheus is configured to scrape the Pushgateway at regular intervals, thus collecting the pushed metrics for
     monitoring and alerting.

- Prometheus: Runs on port 9090 and uses a Prometheus configuration file.
- Node Exporter: (Optional in this exercise) Provides metrics from the host.
- Pushgateway: Runs on port 9091 with persistent storage using a volume

### `docker-compose.yml` Example

```yaml
version: "3.8" # Specifies the version of the Docker Compose file format. 3.8 is stable and widely supported.

services: # Defines the services (containers) to be run.

  prometheus: # Service for Prometheus server.
    image: prom/prometheus:latest # Use the official Prometheus image from Docker Hub, latest version.
    container_name: prometheus # Name the container "prometheus" for easier reference.
    ports:
      - "9090:9090" # Map container's port 9090 to host's port 9090 (Prometheus UI).
    volumes:
      - ./prometheus.yml:/etc/prometheus/prometheus.yml:ro # Mount local prometheus.yml into container (read-only).
      - prometheus_data:/prometheus # Named volume to persist Prometheus data (metrics history) across container restarts.
      - /var/run/docker.sock:/var/run/docker.sock # Mount Docker socket (optional, useful if monitoring Docker itself).
    command:
      - --config.file=/etc/prometheus/prometheus.yml # Tell Prometheus to use the mounted config file.
      - --storage.tsdb.path=/prometheus # Set storage path for Prometheus time series database (TSDB).
    networks:
      - monitoring # Attach this container to the custom "monitoring" network.

  node_exporter: # Service for Node Exporter (collects system metrics like CPU, memory, disk).
    image: prom/node-exporter:latest # Use the official Node Exporter image.
    container_name: node_exporter # Name the container "node_exporter".
    ports:
      - "9100:9100" # Map container's port 9100 to host's port 9100 (Node Exporter metrics endpoint).
    networks:
      - monitoring # Attach to the same "monitoring" network.

  pushgateway: # Service for Pushgateway (allows ephemeral jobs to push metrics to Prometheus).
    image: prom/pushgateway:latest # Use the official Pushgateway image.
    container_name: pushgateway # Name the container "pushgateway".
    ports:
      - "9091:9091" # Map container's port 9091 to host's port 9091 (Pushgateway UI and metrics).
    volumes:
      - ./pushgateway-data:/data # Mount local folder to persist Pushgateway data (metrics it receives).
    command:
      - "--persistence.file=/data/pushgateway.db" # Tell Pushgateway to store data in a file inside the mounted volume.
    networks:
      - monitoring # Attach to the same "monitoring" network.

volumes: # Define named volumes to persist container data.

  prometheus_data: # Volume for Prometheus time series database data.

  pushgateway-data: # Volume for Pushgateway's metric persistence file.

networks: # Define networks used by services.

  monitoring: # Custom bridge network named "monitoring".
...
global: # Global configuration
  scrape_interval: 15s # How often (every 15 seconds) Prometheus scrapes targets by default.

scrape_configs: # List of scrape jobs (i.e., what Prometheus should monitor)

  - job_name: "node_exporter" # Name of the first job
    static_configs: # Static list of targets (no service discovery)
      - targets: ["node_exporter:9100"] # Target is the node_exporter container on port 9100

  - job_name: "pushgateway" # Name of the second job
    static_configs:
      - targets: ["pushgateway:9091"] # Target is the pushgateway container on port 9091
