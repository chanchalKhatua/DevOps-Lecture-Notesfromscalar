...
#### First: What problem this Dockerfile is trying to solve

Goal:

* Install Python dependencies **once**
* Keep final image **small**
* Do **not** keep build tools in final image
...
```
Stage 1 → build everything (heavy image)
Stage 2 → copy only what is needed (light image)
```
...
#### Stage 1: Build stage (EXPLAINED SLOWLY)
...
* Has better compatibility for pip packages
* Name this stage `build-stage`

> Why not Alpine here?
> Many Python wheels **fail to compile on Alpine**.
...
Creates `/app` directory and moves into it.
...
* Installs all Python packages into:
...
* This happens **only in build stage**
* Final image does NOT run this step
...
* All dependencies
* Your script
  exist inside **build-stage**

---

#### Stage 2: Final runtime image (THIS IS THE CONFUSION PART)

```dockerfile
FROM python:3.10-alpine
```

This starts a **brand new image**:

* Lightweight
* Clean
* Nothing from stage 1 exists here
...
Same directory, but **new container filesystem**.
...
* Paste them into final image

So:

* `pip install` is NOT run again
* Only result is copied
