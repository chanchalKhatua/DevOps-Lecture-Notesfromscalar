...
---
Below is a **clean Markdown note** you can directly add to your project README or documentation. It explains **why manager was drained, how services were constrained, and how the architecture works**.

---

# Docker Swarm Architecture and Node Placement

## Overview

This project uses **Docker Swarm** to run a distributed web service with a reverse proxy.

The goal is to separate responsibilities:

* **Manager node** → cluster control + reverse proxy
* **Worker nodes** → application containers

This design improves **cluster stability and production reliability**.
...
Manager Node
   ...
   └ NGINX Reverse Proxy
        ...
        ▼
Overlay Network
   ├ Worker Node 1 → backend container
   └ Worker Node 2 → backend container
```
...
| NGINX           | Reverse proxy for incoming traffic |
| Backend Service | Application containers             |
...
The backend service runs **3 replicas** of the container image.
...
```
backend containers → only worker nodes
manager node → no application workload
```
...
The reverse proxy handles external traffic and forwards requests to the backend service.

Example service configuration:

```bash
docker service create \
--name nginx-proxy_nginx \
--replicas 1 \
--publish 80:80 \
...
```
NGINX runs only on manager node
```
...
The proxy forwards traffic to the Docker Swarm service using **internal DNS service discovery**.
...
http {
    ...
    server {
        ...
        server_name chanchalkhatua.online www.chanchalkhatua.online;
...
Docker DNS (`127.0.0.11`) automatically resolves the **backend service** to available replicas.

---

# Node Availability Management

To prevent workloads from running on the manager node, node availability was changed.
...
New containers → not scheduled
```

This keeps the manager dedicated to cluster control.
...
manager-node          Ready     Active
worker-node-1         Ready     Active
worker-node-2         Ready     Active
```
...
```
backend.1  worker1  Running
backend.2  worker2  Running
backend.3  worker2  Running
```
...
Docker Swarm provides **built-in service load balancing**.
...
NGINX Reverse Proxy
   │
   ▼
Docker Swarm Service (backend)
   ├ Replica 1
   ├ Replica 2
   └ Replica 3
```

Each request may be served by a different container.

---

# Troubleshooting Example

A **504 Gateway Timeout** occurred when NGINX attempted to connect to a backend container that was no longer reachable.
...
```
NGINX pointing to container IP
container restarted
IP changed
```
...
```
Use service name (backend) instead of container IP
```

This leverages Docker Swarm's **service discovery system**.
...
If you want, I can also give you **3 more advanced sections** that make your README **look like a real production DevOps project**:

* **Auto-scaling implementation**
* **Rolling deployments (zero downtime)**
* **Monitoring with Prometheus + Grafana**

Those additions make your GitHub project **much stronger for DevOps interviews.**
