...
kubectl create deployment app2 --image=nginx --port=80
kubectl expose deployment app1 --port=80 --target-port=80 --type=NodePort
kubectl expose deployment app2 --port=80 --target-port=80 --type=NodePort
```
...
```sh
kubectl get ingress
curl -H "Host: example.com" http://<NODE_IP>
curl -H "Host: amazon.com" http://<NODE_IP>/sports
curl -H "Host: amazon.com" http://<NODE_IP>/books
```
