...
## Kubernetes CronJobs
 backoffLimit: 4
 backoffLimit is used in Job configurations to define the number of times a Job will retry before it is considered failed.The Job will retry 4 times before it is marked as failed.
If all retries fail, Kubernetes stops attempting to restart the Job
CronJobs allow scheduling of jobs at specified times, similar to traditional cron jobs in Linux.
