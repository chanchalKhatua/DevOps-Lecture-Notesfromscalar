...
|------------------|----------------------------------|--------------------------------------|
| Overridable      | Yes (via `docker run`)           | No (unless using `--entrypoint`)     |
| Main Use         | Default parameters               | Default executable                   |
| Typical Use Case | Lightweight default commands     | When container is meant to run a fixed app |
